...
import styles from "../../modules/CreatePostModal/CreatePostModal.module.css";
...
const EditPostModal: React.FC<EditPostModalProps> = ({
  ...
  const author = useSelector((state: any) => state.auth.user);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const updatedFromServer = await editPost(postId, caption.trim(), token);

      if (!updatedFromServer || !updatedFromServer.imageUrl) {
        throw new Error("Invalid post data received from server");
      }

      const updated: Post = {
        id: postId,
        caption: updatedFromServer.caption ?? "",
        imageUrl: updatedFromServer.imageUrl,
      };
